...
class FriendsFragment : Fragment(R.layout.fragment_friends) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnDelete.setOnClickListener {
            val name=CurrentUser.currentUser.name
            Toast.makeText(context, "Deleted user: $name", Toast.LENGTH_LONG).show()
            CurrentUser.currentUser.id?.let { it1 -> fundViewModel.deleteUser(it1) }
